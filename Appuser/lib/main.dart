  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      ...
      home: !ready
          ...
          : api.isSignedIn
              ? HomeShell(api: api, initialIndex: homeIndex, onSignedOut: _signedOut)
              ...
              child: AppCard(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      isRegister ? 'Tạo tài khoản SmartParking' : 'Đăng nhập SmartParking',
...
class HomeShell extends StatefulWidget {
  const HomeShell({required this.api, required this.initialIndex, required this.onSignedOut, super.key});
    ...
    return Scaffold(
      ...
      bottomNavigationBar: NavigationBar(
        ...
        destinations: const [
          ...
          NavigationDestination(icon: Icon(Icons.history), selectedIcon: Icon(Icons.history), label: 'Lịch sử'),
          NavigationDestination(icon: Icon(Icons.directions_car_outlined), selectedIcon: Icon(Icons.directions_car), label: 'Xe'),
        ],
      ),
    );
  }
...
class _ProfilePageState extends State<ProfilePage> {
  ...
  Widget build(BuildContext context) {
    ...
    final activePasses = user?.monthlyPasses.where((item) => item.isValidNow).length ?? 0;
      ...
      child: ListView(
        ...
        children: [
                  ...
                  children: [
                    CircleAvatar(
                      ...
                      child: Text(
                        ...
                        style: const TextStyle(color: pine, fontWeight: FontWeight.w800),
                      ),
                    ),
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(user?.fullName.isEmpty == true ? 'Người dùng SmartParking' : user?.fullName ?? '',
                              style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w800)),
                        ],
                      ),
                    ),
                    StatusPill(label: user?.isActive == true ? 'Đang hoạt động' : 'Tạm khóa', color: user?.isActive == true ? pine : Colors.red),
                  ],
                ),
                const SizedBox(height: 18),
                TextField(controller: fullName, decoration: const InputDecoration(labelText: 'Họ tên')),
          ...
          Wrap(
            ...
            runSpacing: 12,
            children: [
              MetricTile(label: 'Xe đã thêm', value: '${user?.vehicles.length ?? 0}', icon: Icons.directions_car),
              MetricTile(label: 'Vé đang hiệu lực', value: '$activePasses', icon: Icons.confirmation_number),
            ],
          ),
          const SizedBox(height: 16),
          AppCard(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Vé tháng của tôi', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w800)),
      ...
      child: ListView(
        ...
        children: [
          ...
          if (transactions.isEmpty)
            const AppCard(padding: EdgeInsets.all(16), child: EmptyText('Chưa có giao dịch.'))
          else
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          ...
          if (items.isEmpty)
            const AppCard(padding: EdgeInsets.all(18), child: EmptyText('Chưa có lịch sử gửi xe.'))
          else
            for (final item in items)
              ParkingHistoryTile(item: item),
        ],
      ),
    );
  }
...
class _VehiclesPageState extends State<VehiclesPage> {
  ...
  Future<void> load() async {
    try {
      final results = await Future.wait([widget.api.getProfile(), widget.api.getVehicles()]);
        ...
        children: [
          ...
          AppCard(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.stretch,
              children: [
                Text('Thêm xe mới', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w800)),
                const SizedBox(height: 12),
                TextField(controller: plate, decoration: const InputDecoration(labelText: 'Biển số')),
                const SizedBox(height: 12),
                SegmentedButton<String>(
                  segments: const [
                    ButtonSegment(value: 'Motorbike', icon: Icon(Icons.two_wheeler), label: Text('Xe máy')),
                    ButtonSegment(value: 'Car', icon: Icon(Icons.directions_car), label: Text('Ô tô')),
                  ],
                  selected: {vehicleType},
                  onSelectionChanged: (value) => setState(() => vehicleType = value.first),
                ),
                const SizedBox(height: 12),
                TextField(controller: brand, decoration: const InputDecoration(labelText: 'Hãng xe')),
                const SizedBox(height: 12),
                TextField(controller: color, decoration: const InputDecoration(labelText: 'Màu xe')),
                SwitchListTile(
          ...
          const SizedBox(height: 18),
          if (vehicles.isEmpty)
            const AppCard(padding: EdgeInsets.all(18), child: EmptyText('Bạn cần thêm xe trước khi mua vé tháng.'))
          else ...[
            Text('Danh sách xe', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w800)),
            ...
            AppCard(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 12),
                  TextField(controller: ownerName, decoration: const InputDecoration(labelText: 'Tên chủ xe')),
                  const SizedBox(height: 12),
                  TextField(
                    controller: ownerPhone,
                    decoration: const InputDecoration(labelText: 'Số điện thoại chủ xe'),
                    keyboardType: TextInputType.phone,
                  ),
                  const SizedBox(height: 12),
                  TextField(
                    ...
                    enabled: false,
                    decoration: const InputDecoration(labelText: 'Biển số xe mặc định'),
                  ),
                  const SizedBox(height: 8),
                  StepperControl(
                    label: 'Số tháng',
                  ...
                  ReadOnlyLine(label: 'Hiệu lực', value: '${shortDateFormat.format(from)} - ${shortDateFormat.format(to)}'),
                  ReadOnlyLine(label: 'Tổng thanh toán', value: currencyFormat.format(total)),
                  const SizedBox(height: 12),
                  FilledButton.icon(
  ...
  Future<void> buyPassForDefaultVehicle() async {
    ...
    if (ownerName.text.trim().isEmpty || ownerPhone.text.trim().isEmpty) {
      showMessage(context, 'Vui lòng cập nhật đầy đủ tên và số điện thoại chủ xe.');
      return;
    }
    ...
    try {
      ...
      if (rawUrl == null || rawUrl.toString().isEmpty) {
        if (!mounted) return;
        showMessage(context, payment['message']?.toString() ?? 'Không nhận được link MoMo.');
        return;
      }
...
class AppCard extends StatelessWidget {
  const AppCard({required this.child, this.padding = EdgeInsets.zero, super.key});
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(title, style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w800, color: ink)),
          if (subtitle != null) ...[
...
class CompactPassTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ListTile(
      contentPadding: EdgeInsets.zero,
      leading: Icon(pass.isValidNow ? Icons.verified : Icons.event_busy, color: pass.isValidNow ? pine : Colors.black45),
      title: Text(pass.licensePlate, style: const TextStyle(fontWeight: FontWeight.w800)),
      subtitle: Text('${shortDateFormat.format(pass.validFrom)} - ${shortDateFormat.format(pass.validTo)}'),
      trailing: StatusPill(label: pass.isValidNow ? 'Hiệu lực' : 'Hết hạn', color: pass.isValidNow ? pine : Colors.black45),
    );
  }
}
      ...
      child: AppCard(
        ...
        child: ListTile(
          ...
          leading: CircleAvatar(
            ...
            child: Icon(positive ? Icons.add : Icons.remove, color: positive ? pine : Colors.red),
          ),
          title: Text(item.description.isEmpty ? 'Thanh toán SmartParking' : item.description),
          subtitle: Text(dateFormat.format(item.createdAt)),
          trailing: Text(currencyFormat.format(item.amount), style: const TextStyle(fontWeight: FontWeight.w800)),
        ),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: AppCard(
        ...
        child: ListTile(
          contentPadding: EdgeInsets.zero,
          leading: const CircleAvatar(backgroundColor: mint, child: Icon(Icons.local_parking, color: pine)),
          title: Text(item.licensePlate, style: const TextStyle(fontWeight: FontWeight.w800)),
          subtitle: Text(
          ...
          trailing: Column(
            ...
            children: [
              Text(item.status, style: const TextStyle(fontWeight: FontWeight.w700)),
      ...
      child: AppCard(
        ...
        child: ListTile(
          ...
          leading: CircleAvatar(
            ...
            child: Icon(vehicle.vehicleType == 'Car' ? Icons.directions_car : Icons.two_wheeler, color: selected ? pine : Colors.black54),
          ),
          title: Text(vehicle.licensePlate, style: const TextStyle(fontWeight: FontWeight.w800)),
          subtitle: Text('${vehicle.brand} - ${vehicle.color}'),
          trailing: vehicle.isDefault
              ? const StatusPill(label: 'Mặc định', color: pine)
              : TextButton.icon(onPressed: onSetDefault, icon: const Icon(Icons.star_outline), label: const Text('Chọn')),
        ),
      ),
    );
  }
...
class EmptyText extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Text(text, textAlign: TextAlign.center, style: const TextStyle(color: Colors.black54));
  }
}
